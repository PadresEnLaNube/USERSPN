...
class USERSPN_Common {
    ...
    if (!wp_style_is($this->plugin_name . '-fancybox', 'enqueued')) {
			wp_enqueue_style($this->plugin_name . '-fancybox', USERSPN_URL . 'assets/css/fancybox.min.css', [], $this->version, 'all');
    }
    ...
    if(!wp_script_is($this->plugin_name . '-fancybox', 'enqueued')) {
			wp_enqueue_script($this->plugin_name . '-fancybox', USERSPN_URL . 'assets/js/fancybox.min.js', ['jquery'], $this->version, false, ['in_footer' => true, 'strategy' => 'defer']);
    }
    ...
    if(!wp_script_is($this->plugin_name . '-datatables', 'enqueued')) {
		...
		wp_localize_script($this->plugin_name, 'userspn_ajax', [
			...
			'ajax_nonce' => wp_create_nonce('userspn-nonce'),
		]);
		...
		wp_localize_script($this->plugin_name, 'userspn_i18n', [
			...
			'complete_required_fields' => esc_html(__('Please complete all the required fields (*) of your profile', 'userspn')),
